    ...
    fun generate_task(start: Long, end: Long, ctx: Context, ptype : Push2Plot.PlotType) : Runnable {
        return Runnable {
            try {
                ...
                Push2Plot.place_data_in_view(avg_elem, "avg:", String.format("%.1f", avg), UserData.get_unit(ctx))
                Push2Plot.place_data_in_view(std_dev_elem, "stddev:", String.format("%.1f", stddev), UserData.get_unit(ctx))
