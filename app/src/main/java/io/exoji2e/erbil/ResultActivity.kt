...
class ResultActivity : ErbilActivity() {
    ...
    private fun showLayout() {
        val task = Runnable {
            ...
            DebugTV.text = String.format("%s\n%s\n%s", fmt(resources.getString(R.string.history), RawParser.history(raw_data)[31].value),
                    fmt(resources.getString(R.string.recent), last),
                    fmt(resources.getString(R.string.guess), predict))
